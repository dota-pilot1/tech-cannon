...
export const issueApi = {
  // 이슈 목록 조회
  getIssues: async (filters?: IssueFilters & { page?: number; limit?: number }): Promise<IssueListResponse> => {
    const { data } = await apiClient.get<IssueListResponse>('/issues', { params: filters })
    return data
  },
  ...
  // 이슈 수정
  updateIssue: async (id: number, request: UpdateIssueRequest): Promise<Issue> => {
