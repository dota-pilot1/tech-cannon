...
export function ChatRoomsPage() {
  ...
  const [newRoomType, setNewRoomType] = useState<"TEAM" | "PROJECT" | "DIRECT">(
    "TEAM",
  );
  ...
  const handleRoomClick = async (roomId: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (err?.response?.status === 409 || err?.message?.includes("이미")) {
        navigate({ to: "/chat/$roomId", params: { roomId: String(roomId) } });
      } else {
  ...
  const handleEditClick = (
    e: React.MouseEvent,
    room: { id: number; name: string; roomType: string | null | undefined },
  ) => {
    e.stopPropagation();
    setEditRoomId(room.id);
    setEditRoomName(room.name);
    setEditRoomType(resolveRoomType(room.roomType));
  ...
  const handleEditSave = () => {
    ...
    updateRoomMutation.mutate({
      id: editRoomId,
      room: { name: editRoomName, roomType: editRoomType },
    });
  };
  ...
  const handleDeleteAllRooms = async () => {
    const confirmed = await confirm({
      title: "모든 채팅방 삭제",
      description:
        "정말로 모든 채팅방을 삭제하시겠습니까? 이 작업은 되돌릴 수 없으며, 모든 참가자 정보도 함께 삭제됩니다.",
      confirmText: "삭제",
      cancelText: "취소",
      variant: "destructive",
    });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => setIsCreateDialogOpen(true)}>
              <Plus className="w-4 h-4 mr-2" />새 채팅방
            </Button>
          </div>
        </div>
      </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>채팅방 정보를 수정해주세요</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-room-type">채팅방 유형</Label>
              <Select
                value={editRoomType}
                onValueChange={(value: "TEAM" | "PROJECT" | "DIRECT") =>
                  setEditRoomType(value)
                }
              >
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {updateRoomMutation.isPending ? "저장 중..." : "저장"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
