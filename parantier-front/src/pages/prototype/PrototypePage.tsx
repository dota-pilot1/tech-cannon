...
import {
  useProtoFolders,
  useProtoPostDetail,
  useSaveProtoMutation,
  useDeleteProtoMutation,
  useCreateProtoFolderMutation,
  useRenameProtoFolderMutation,
  useDeleteProtoFolderMutation,
} from "@/features/prototype/usePrototype";
import { protoApi } from "@/entities/prototype/protoApi";
import type {
  ProtoFolder,
  ProtoPost,
  ProtoBlock,
} from "@/entities/prototype/prototype.types";
import {
  buildProtoTree,
  parseTags,
} from "@/entities/prototype/prototype.types";
...
import type { TaskBlock, TaskPost } from "@/features/task/types/task.types";
...
export function PrototypePage() {
  ...
  const { data: folders = [] } = useProtoFolders();
  const { roots, children: folderChildren } = useMemo(
    () => buildProtoTree(folders),
    [folders],
  );
  ...
  const [blocks, setBlocks] = useState<ProtoBlock[]>([]);
  ...
  const postsByFolder = useMemo(() => {
    const map = new Map<number, ProtoPost[]>();
  ...
  const { data: postDetail } = useProtoPostDetail(selectedPostId, !isEditing);

  const saveMutation = useSaveProtoMutation(selectedPostId, (newId: number) => {
    setSelectedPostId(newId);
    setIsEditing(false);
  });

  const deleteMutation = useDeleteProtoMutation(() => {
    setSelectedPostId(null);
    setIsEditing(false);
  });

  const createFolderMutation = useCreateProtoFolderMutation(
    (parentId: number | null) => {
  ...
  const renameFolderMutation = useRenameProtoFolderMutation(() => {
    setEditingFolderId(null);
  });

  const deleteFolderMutation = useDeleteProtoFolderMutation(() => {
    setSelectedFolderId(null);
    setSelectedPostId(null);
  });

  const createDocMutation = useSaveProtoMutation(null, (newId: number) => {
  ...
  const handlePostClick = (post: ProtoPost) => {
  ...
  const handleEdit = () => {
    ...
    setFormTags(postDetail.tags ?? "");
    setFormIsPinned(postDetail.isPinned ?? false);
    setBlocks(
  ...
  const handleDelete = async () => {
    ...
    const ok = await confirm({
      title: "삭제 확인",
      description: "이 Prototype 문서를 삭제하시겠습니까?",
      variant: "destructive",
    });
  ...
  // 폴더 렌더링
  const renderFolder = (folder: ProtoFolder, depth = 0) => {
    ...
    return (
      <div key={folder.id} className={depth > 0 ? "ml-4" : ""}>
        ...
        {isExpanded && (
          <>
            {subFolders.map((sub: ProtoFolder) => renderFolder(sub, depth + 1))}
    ...
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="bg-card rounded border mb-4">
        <div className="p-3 border-b bg-muted/30">
          <h1 className="text-lg font-bold text-foreground">Prototype</h1>
        </div>
      </div>
      ...
      >
                  ...
                  <div>
                    {matchedFolders.length > 0 && (
                      <>
                        ...
                        {matchedFolders.map((f: ProtoFolder) => (
                          <div
                            key={f.id}
                    ...
                    {allPosts.length > 0 && (
                      <>
                        ...
                        {allPosts.map((p: ProtoPost & { folderId: number }) => {
                          return (
                            <div
                              key={p.id}
        ...
        >
          <div className="w-[1px] h-full bg-border group-hover:bg-primary/50 group-active:bg-primary transition-colors"></div>
        </div>
            ...
            {isEditing ? (
              <div className="flex flex-col gap-3">
                ...
                <input
                  ...
                  className="w-full border border-border rounded px-3 py-1.5 text-sm bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                />
                {/* isPinned 체크박스 */}
                ...
                <TaskBlockEditor
                  ...
                  blocks={blocks as unknown as TaskBlock[]}
                  setBlocks={
                    setBlocks as unknown as (blocks: TaskBlock[]) => void
                  }
                />
              </div>
            ) : postDetail ? (
              <div className="flex flex-col gap-3">
                {/* 태그 배지 표시 */}
                {parseTags(postDetail.tags).length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {parseTags(postDetail.tags).map((tag: string) => (
                      <span
                        key={tag}
                ...
                <TaskBlockViewer post={postDetail as unknown as TaskPost} />
              </div>
            ) : (
              <div className="flex items-center justify-center h-[300px] text-muted-foreground text-sm">
                {selectedFolderId
                  ? "폴더 hover 후 + 버튼으로 새 문서 추가"
                  : "좌측에서 프로토타입 문서를 선택하거나 새 문서를 작성하세요."}
              </div>
            )}
          </div>
        </div>
