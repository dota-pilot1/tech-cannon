...
// ─────────────────────────────────────────────
function BlockViewer({ block }: { block: SecurityBlock }) {
...
export default function SecurityPage() {
  ...
  const [cat1Width, setCat1Width] = useState(() => {
    const saved = localStorage.getItem("security-cat1-width");
    return saved ? Number(saved) : 192;
  });
  const [cat2Width, setCat2Width] = useState(() => {
    const saved = localStorage.getItem("security-cat2-width");
  ...
  const startResize1 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat1Width(next);
        localStorage.setItem("security-cat1-width", String(next));
      };
      const onUp = () => {
  ...
  const startResize2 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat2Width(next);
        localStorage.setItem("security-cat2-width", String(next));
      };
      const onUp = () => {
  ...
  const [editBlocks, setEditBlocks] = useState<SecurityBlock[]>([]);
  ...
  // Queries
  // ─────────────────────────────────────────────
  const { data: categories = [] } = useQuery<SecurityCategory[]>({
    queryKey: ["security", "categories"],
    queryFn: securityApi.getCategories,
  });

  const { data: sections = [] } = useQuery<SecuritySection[]>({
    queryKey: ["security", "sections", selectedCategoryId],
    queryFn: () => securityApi.getSections(selectedCategoryId!),
    enabled: !!selectedCategoryId,
  });

  const { data: blocks = [] } = useQuery<SecurityBlock[]>({
    queryKey: ["security", "blocks", selectedSectionId],
    queryFn: () => securityApi.getBlocks(selectedSectionId!),
  ...
  const saveMutation = useMutation({
    mutationFn: () => securityApi.saveBlocks(selectedSectionId!, editBlocks),
    onSuccess: () => {
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: ({ id, name }: { id: number; name: string }) => {
      ...
      return securityApi.updateCategory(id, {
        name,
  ...
  const reorderCategoryMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      securityApi.reorderCategories(items),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["security", "categories"] });
    },
  ...
  const reorderSectionMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      securityApi.reorderSections(items),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["security", "sections", selectedCategoryId],
      });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => securityApi.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["security", "categories"] });
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => securityApi.deleteSection(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["security", "sections", selectedCategoryId],
      });
  ...
  const renameSectionMutation = useMutation({
    mutationFn: ({ id, title }: { id: number; title: string }) => {
      ...
      return securityApi.updateSection(id, {
        title,
            ...
            {isEditing ? (
              <div>
                <TaskBlockEditor
                  ...
                  setBlocks={(b) => setEditBlocks(b as SecurityBlock[])}
                />
                {/* 저장 / 취소 */}
