...
import { frontendApi } from "@/features/frontend/api/frontendApi";
import type {
  FrontCategory,
  FrontSection,
  FrontBlock,
} from "@/features/frontend/api/frontendApi";
...
// ─────────────────────────────────────────────
function BlockViewer({ block }: { block: FrontBlock }) {
...
export default function FrontendPage() {
  ...
  const [cat1Width, setCat1Width] = useState(() => {
    const saved = localStorage.getItem("front-cat1-width");
    return saved ? Number(saved) : 192;
  });
  const [cat2Width, setCat2Width] = useState(() => {
    const saved = localStorage.getItem("front-cat2-width");
  ...
  const startResize1 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat1Width(next);
        localStorage.setItem("front-cat1-width", String(next));
      };
      const onUp = () => {
  ...
  const startResize2 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat2Width(next);
        localStorage.setItem("front-cat2-width", String(next));
      };
      const onUp = () => {
  ...
  const [editBlocks, setEditBlocks] = useState<FrontBlock[]>([]);
  ...
  // Queries
  // ─────────────────────────────────────────────
  const { data: categories = [] } = useQuery<FrontCategory[]>({
    queryKey: ["front", "categories"],
    queryFn: frontendApi.getCategories,
  });

  const { data: sections = [] } = useQuery<FrontSection[]>({
    queryKey: ["front", "sections", selectedCategoryId],
    queryFn: () => frontendApi.getSections(selectedCategoryId!),
    enabled: !!selectedCategoryId,
  });

  const { data: blocks = [] } = useQuery<FrontBlock[]>({
    queryKey: ["front", "blocks", selectedSectionId],
    queryFn: () => frontendApi.getBlocks(selectedSectionId!),
  ...
  const saveMutation = useMutation({
    mutationFn: () => frontendApi.saveBlocks(selectedSectionId!, editBlocks),
    onSuccess: () => {
  ...
  const reorderCategoryMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      frontendApi.reorderCategories(items),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["front", "categories"] });
    },
  ...
  const reorderSectionMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      frontendApi.reorderSections(items),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["front", "sections", selectedCategoryId],
      });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => frontendApi.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["front", "categories"] });
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => frontendApi.deleteSection(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["front", "sections", selectedCategoryId],
      });
  ...
  const renameSectionMutation = useMutation({
    mutationFn: ({ id, title }: { id: number; title: string }) => {
      ...
      return frontendApi.updateSection(id, {
        title,
            ...
            {isEditing ? (
              <div>
                <TaskBlockEditor
                  ...
                  setBlocks={(b) => setEditBlocks(b as FrontBlock[])}
                />
                {/* 저장 / 취소 */}
