...
import { architectureApi } from "@/features/architecture/api/architectureApi";
import type {
  ArchCategory,
  ArchSection,
  ArchBlock,
} from "@/features/architecture/api/architectureApi";
...
// ─────────────────────────────────────────────
function BlockViewer({ block }: { block: ArchBlock }) {
...
export default function ArchitecturePage() {
  ...
  const [cat1Width, setCat1Width] = useState(() => {
    const saved = localStorage.getItem("arch-cat1-width");
    return saved ? Number(saved) : 192;
  });
  const [cat2Width, setCat2Width] = useState(() => {
    const saved = localStorage.getItem("arch-cat2-width");
  ...
  const startResize1 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat1Width(next);
        localStorage.setItem("arch-cat1-width", String(next));
      };
      const onUp = () => {
  ...
  const startResize2 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat2Width(next);
        localStorage.setItem("arch-cat2-width", String(next));
      };
      const onUp = () => {
  ...
  const [editBlocks, setEditBlocks] = useState<ArchBlock[]>([]);
  ...
  // Queries
  // ─────────────────────────────────────────────
  const { data: categories = [] } = useQuery<ArchCategory[]>({
    queryKey: ["arch", "categories"],
    queryFn: architectureApi.getCategories,
  });

  const { data: sections = [] } = useQuery<ArchSection[]>({
    queryKey: ["arch", "sections", selectedCategoryId],
    queryFn: () => architectureApi.getSections(selectedCategoryId!),
    enabled: !!selectedCategoryId,
  });

  const { data: blocks = [] } = useQuery<ArchBlock[]>({
    queryKey: ["arch", "blocks", selectedSectionId],
    queryFn: () => architectureApi.getBlocks(selectedSectionId!),
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      architectureApi.saveBlocks(selectedSectionId!, editBlocks),
    onSuccess: () => {
  ...
  const addCategoryMutation = useMutation({
    mutationFn: (name: string) =>
      architectureApi.createCategory({
        name,
        icon: "Folder",
        emoji: "",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["arch", "categories"] });
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: ({ id, name }: { id: number; name: string }) => {
      ...
      return architectureApi.updateCategory(id, {
        name,
  ...
  const reorderCategoryMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      architectureApi.reorderCategories(items),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["arch", "categories"] });
    },
  ...
  const reorderSectionMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      architectureApi.reorderSections(items),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["arch", "sections", selectedCategoryId],
      });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => architectureApi.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["arch", "categories"] });
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => architectureApi.deleteSection(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["arch", "sections", selectedCategoryId],
      });
            ...
            {isEditing ? (
              <div>
                <TaskBlockEditor
                  ...
                  setBlocks={(b) => setEditBlocks(b as ArchBlock[])}
                />
                {/* 저장 / 취소 */}
