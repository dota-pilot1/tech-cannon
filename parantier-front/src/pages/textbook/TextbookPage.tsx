...
import { textbookApi } from "@/features/textbook/api/textbookApi";
import type {
  TextbookCategory,
  TextbookSection,
  TextbookBlock,
} from "@/features/textbook/api/textbookApi";
...
// ─────────────────────────────────────────────
function BlockViewer({ block }: { block: TextbookBlock }) {
...
export default function TextbookPage() {
  ...
  const [cat1Width, setCat1Width] = useState(() => {
    const saved = localStorage.getItem("textbook-cat1-width");
    return saved ? Number(saved) : 192;
  });
  const [cat2Width, setCat2Width] = useState(() => {
    const saved = localStorage.getItem("textbook-cat2-width");
  ...
  const startResize1 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat1Width(next);
        localStorage.setItem("textbook-cat1-width", String(next));
      };
      const onUp = () => {
  ...
  const startResize2 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat2Width(next);
        localStorage.setItem("textbook-cat2-width", String(next));
      };
      const onUp = () => {
  ...
  const [editBlocks, setEditBlocks] = useState<TextbookBlock[]>([]);
  ...
  // Queries
  // ─────────────────────────────────────────────
  const { data: categories = [] } = useQuery<TextbookCategory[]>({
    queryKey: ["textbook", "categories"],
    queryFn: textbookApi.getCategories,
  });

  const { data: sections = [] } = useQuery<TextbookSection[]>({
    queryKey: ["textbook", "sections", selectedCategoryId],
    queryFn: () => textbookApi.getSections(selectedCategoryId!),
    enabled: !!selectedCategoryId,
  });

  const { data: blocks = [] } = useQuery<TextbookBlock[]>({
    queryKey: ["textbook", "blocks", selectedSectionId],
    queryFn: () => textbookApi.getBlocks(selectedSectionId!),
  ...
  const saveMutation = useMutation({
    mutationFn: () => textbookApi.saveBlocks(selectedSectionId!, editBlocks),
    onSuccess: () => {
  ...
  const addCategoryMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["textbook", "categories"] });
  ...
  const reorderCategoryMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      textbookApi.reorderCategories(items),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["textbook", "categories"] });
    },
  ...
  const reorderSectionMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      textbookApi.reorderSections(items),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["textbook", "sections", selectedCategoryId],
      });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => textbookApi.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["textbook", "categories"] });
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => textbookApi.deleteSection(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["textbook", "sections", selectedCategoryId],
      });
            ...
            {isEditing ? (
              <div>
                <TaskBlockEditor
                  ...
                  setBlocks={(b) => setEditBlocks(b as TextbookBlock[])}
                />
                {/* 저장 / 취소 */}
