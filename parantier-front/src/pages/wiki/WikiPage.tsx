...
import type {
  ...
  WikiPost,
  WikiBlock,
} from "@/entities/wiki/wiki.types";
import { buildWikiTree, parseTags } from "@/entities/wiki/wiki.types";
...
export function WikiPage() {
  ...
  // Wiki 전용: 읽기 모드 기본 (isEditing 초기값 false)
  const [isEditing, setIsEditing] = useState(false);
  const [formTitle, setFormTitle] = useState("");
  const [blocks, setBlocks] = useState<WikiBlock[]>([]);

  // Wiki 전용: tags / isPinned 편집 상태
  ...
  const postsByFolder = useMemo(() => {
    const map = new Map<number, WikiPost[]>();
  ...
  const { data: postDetail } = useWikiPostDetail(selectedPostId, !isEditing);

  const saveMutation = useSaveWikiMutation(selectedPostId, (newId: number) => {
    setSelectedPostId(newId);
    setIsEditing(false);
  });

  const deleteMutation = useDeleteWikiMutation(() => {
    setSelectedPostId(null);
    setIsEditing(false);
  });

  const createFolderMutation = useCreateWikiFolderMutation(
    (parentId: number | null) => {
  ...
  const renameFolderMutation = useRenameWikiFolderMutation(() => {
    setEditingFolderId(null);
  });

  const deleteFolderMutation = useDeleteWikiFolderMutation(() => {
    setSelectedFolderId(null);
    setSelectedPostId(null);
  });

  const createDocMutation = useSaveWikiMutation(null, (newId: number) => {
  ...
  const handlePostClick = (post: WikiPost) => {
  ...
  const handleDelete = async () => {
    ...
    const ok = await confirm({
      title: "삭제 확인",
      description: "이 Wiki 문서를 삭제하시겠습니까?",
      variant: "destructive",
    });
  ...
  const renderFolder = (folder: WikiFolder, depth = 0) => {
    ...
    return (
      <div key={folder.id} className={depth > 0 ? "ml-4" : ""}>
        ...
        {isExpanded && (
          <>
            {subFolders.map((sub: WikiFolder) => renderFolder(sub, depth + 1))}
    ...
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="bg-card rounded border mb-4">
        <div className="p-3 border-b bg-muted/30">
          <h1 className="text-lg font-bold text-foreground">Tech Wiki</h1>
        </div>
      </div>
                  ...
                  <div>
                    ...
                    {allPosts.length > 0 && (
                      <>
                        ...
                        {allPosts.map((p: WikiPost & { folderId: number }) => {
                          return (
                            <div
                              key={p.id}
