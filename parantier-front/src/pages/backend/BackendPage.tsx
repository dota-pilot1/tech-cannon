...
// ─────────────────────────────────────────────
function BlockViewer({ block }: { block: CoreBlock }) {
...
export default function CorePage() {
  ...
  const [cat1Width, setCat1Width] = useState(() => {
    const saved = localStorage.getItem("core-cat1-width");
    return saved ? Number(saved) : 192;
  });
  const [cat2Width, setCat2Width] = useState(() => {
    const saved = localStorage.getItem("core-cat2-width");
  ...
  const startResize1 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat1Width(next);
        localStorage.setItem("core-cat1-width", String(next));
      };
      const onUp = () => {
  ...
  const startResize2 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat2Width(next);
        localStorage.setItem("core-cat2-width", String(next));
      };
      const onUp = () => {
  ...
  const [editBlocks, setEditBlocks] = useState<CoreBlock[]>([]);
  ...
  // Queries
  // ─────────────────────────────────────────────
  const { data: categories = [] } = useQuery<CoreCategory[]>({
    queryKey: ["core", "categories"],
    queryFn: coreApi.getCategories,
  });

  const { data: sections = [] } = useQuery<CoreSection[]>({
    queryKey: ["core", "sections", selectedCategoryId],
    queryFn: () => coreApi.getSections(selectedCategoryId!),
    enabled: !!selectedCategoryId,
  });

  const { data: blocks = [] } = useQuery<CoreBlock[]>({
    queryKey: ["core", "blocks", selectedSectionId],
    queryFn: () => coreApi.getBlocks(selectedSectionId!),
  ...
  const saveMutation = useMutation({
    mutationFn: () => coreApi.saveBlocks(selectedSectionId!, editBlocks),
    onSuccess: () => {
  ...
  const addCategoryMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["core", "categories"] });
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: ({ id, name }: { id: number; name: string }) => {
      ...
      return coreApi.updateCategory(id, {
        name,
  ...
  const reorderCategoryMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      coreApi.reorderCategories(items),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["core", "categories"] });
    },
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => coreApi.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["core", "categories"] });
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => coreApi.deleteSection(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["core", "sections", selectedCategoryId],
      });
  ...
  const renameSectionMutation = useMutation({
    mutationFn: ({ id, title }: { id: number; title: string }) => {
      ...
      return coreApi.updateSection(id, {
        title,
            ...
            {isEditing ? (
              <div>
                <TaskBlockEditor
                  ...
                  setBlocks={(b) => setEditBlocks(b as CoreBlock[])}
                />
                {/* 저장 / 취소 */}
