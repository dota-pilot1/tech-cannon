...
import { devopsApi } from "@/features/devops/api/devopsApi";
import type {
  DevOpsCategory,
  DevOpsSection,
  DevOpsBlock,
} from "@/features/devops/api/devopsApi";
...
// ─────────────────────────────────────────────
function BlockViewer({ block }: { block: DevOpsBlock }) {
...
export default function DevOpsPage() {
  ...
  const [cat1Width, setCat1Width] = useState(() => {
    const saved = localStorage.getItem("devops-cat1-width");
    return saved ? Number(saved) : 192;
  });
  const [cat2Width, setCat2Width] = useState(() => {
    const saved = localStorage.getItem("devops-cat2-width");
  ...
  const startResize1 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat1Width(next);
        localStorage.setItem("devops-cat1-width", String(next));
      };
      const onUp = () => {
  ...
  const startResize2 = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setCat2Width(next);
        localStorage.setItem("devops-cat2-width", String(next));
      };
      const onUp = () => {
  ...
  const [editBlocks, setEditBlocks] = useState<DevOpsBlock[]>([]);
  ...
  // Queries
  // ─────────────────────────────────────────────
  const { data: categories = [] } = useQuery<DevOpsCategory[]>({
    queryKey: ["devops", "categories"],
    queryFn: devopsApi.getCategories,
  });

  const { data: sections = [] } = useQuery<DevOpsSection[]>({
    queryKey: ["devops", "sections", selectedCategoryId],
    queryFn: () => devopsApi.getSections(selectedCategoryId!),
    enabled: !!selectedCategoryId,
  });

  const { data: blocks = [] } = useQuery<DevOpsBlock[]>({
    queryKey: ["devops", "blocks", selectedSectionId],
    queryFn: () => devopsApi.getBlocks(selectedSectionId!),
  ...
  const saveMutation = useMutation({
    mutationFn: () => devopsApi.saveBlocks(selectedSectionId!, editBlocks),
    onSuccess: () => {
  ...
  const addCategoryMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["devops", "categories"] });
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: ({ id, name }: { id: number; name: string }) => {
      ...
      return devopsApi.updateCategory(id, {
        name,
  ...
  const reorderCategoryMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      devopsApi.reorderCategories(items),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["devops", "categories"] });
    },
  ...
  const reorderSectionMutation = useMutation({
    mutationFn: (items: { id: number; orderNum: number }[]) =>
      devopsApi.reorderSections(items),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["devops", "sections", selectedCategoryId],
      });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => devopsApi.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["devops", "categories"] });
  ...
  const deleteSectionMutation = useMutation({
    mutationFn: (id: number) => devopsApi.deleteSection(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["devops", "sections", selectedCategoryId],
      });
  ...
  const renameSectionMutation = useMutation({
    mutationFn: ({ id, title }: { id: number; title: string }) => {
      ...
      return devopsApi.updateSection(id, {
        title,
            ...
            {isEditing ? (
              <div>
                <TaskBlockEditor
                  ...
                  setBlocks={(b) => setEditBlocks(b as DevOpsBlock[])}
                />
                {/* 저장 / 취소 */}
