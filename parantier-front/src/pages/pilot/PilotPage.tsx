...
} from "@/features/pilot/usePilot";
import { taskApi } from "@/entities/pilot/pilotApi";
import type {
  TaskFolder,
  TaskPost,
  TaskBlock,
} from "@/entities/pilot/pilot.types";
import { buildTree } from "@/entities/pilot/pilot.types";
...
export function PilotPage() {
  ...
  const { data: folders = [] } = usePilotFolders();
  ...
  const { data: postDetail } = usePilotPostDetail(selectedPostId, !isEditing);

  const saveMutation = useSavePilotMutation(
    selectedFolderId,
    selectedPostId,
    (newId) => {
  ...
  const deleteMutation = useDeletePilotMutation(selectedFolderId, () => {
  ...
  const createDocMutation = useSavePilotMutation(
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="bg-card rounded border mb-4">
        <div className="p-3 border-b bg-muted/30">
          <h1 className="text-lg font-bold text-foreground">파일럿 관리</h1>
        </div>
      </div>
