...
export function AuthoritiesPage() {
  ...
  const { data: categories = [], isLoading: isCategoriesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      data.forEach((cat, index) => {
        console.log(
          `  [${index}] ID: ${cat.id}, Name: ${cat.name}, Description: ${cat.description}`,
        );
      });
      console.log("================================");
      return data;
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: CreateAuthorityRequest }) =>
  ...
  const groupedByCategory = categories.reduce(
    (acc, category) => {
      acc[category.name] = {
        ...
        authorities: authorities.filter(
          (auth) => auth.categoryId === category.id,
        ),
      };
      return acc;
    },
    {} as Record<
      string,
      { categoryId: number; authorities: typeof authorities }
    >,
  );
        ...
        <div className="grid grid-cols-2 gap-6">
          {Object.entries(groupedByCategory)
            .sort(([, a], [, b]) => {
              ...
              return (
                new Date(categoryB.createdAt).getTime() -
                new Date(categoryA.createdAt).getTime()
              );
            })
