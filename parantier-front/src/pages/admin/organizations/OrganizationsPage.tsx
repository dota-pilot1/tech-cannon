...
function AuthorityAssignmentDialog({
  ...
  >([]);
  const [selectedAuthorities, setSelectedAuthorities] = useState<Set<number>>(
    new Set(),
  );
  const [selectedLeftItems, setSelectedLeftItems] = useState<Set<number>>(
    new Set(),
  );
  const [selectedRightItems, setSelectedRightItems] = useState<Set<number>>(
    new Set(),
  );
  const [expandedLeftCategories, setExpandedLeftCategories] = useState<
    Set<string>
  >(new Set());
  const [expandedRightCategories, setExpandedRightCategories] = useState<
    Set<string>
  >(new Set());
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setSelectedAuthorities(
        new Set(currentAuthorities.map((a) => a.authorityId)),
      );
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await adminApi.updateUserAuthorities(
        userId,
        Array.from(selectedAuthorities),
      );
  ...
  // 왼쪽 패널: 사용자가 선택한 권한
  const selectedAuthorityList = allAuthorities.filter((a) =>
    selectedAuthorities.has(a.id),
  );

  // 오른쪽 패널: 선택되지 않은 권한
  const availableAuthorityList = allAuthorities.filter(
    (a) => !selectedAuthorities.has(a.id),
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[98vw] w-[98vw] max-h-[98vh] h-[98vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            권한을 클릭하여 할당하거나 제거할 수 있습니다. 카테고리별로 분류되어
            있습니다.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex items-center justify-between px-4 py-3 bg-primary/10 rounded-lg mb-3">
                ...
                <span className="text-sm font-medium text-primary">
                  {Object.keys(selectedGrouped).length}개 카테고리 /{" "}
                  {selectedAuthorities.size}개 권한
                </span>
              </div>
              <div className="border rounded-lg flex-1 overflow-y-auto">
                {selectedAuthorityList.length === 0 ? (
                  ...
                  <div className="p-3 space-y-3">
                    {Object.entries(selectedGrouped).map(
                      ([categoryId, authorities]) => {
                        const isExpanded =
                          expandedLeftCategories.has(categoryId);
                            ...
                            {isExpanded && (
                              <div className="space-y-1 ml-4">
                                {authorities.map((authority) => (
                                  ...
                                  >
                                    <Checkbox
                                      checked={selectedLeftItems.has(
                                        authority.id,
                                      )}
                                      onCheckedChange={() =>
                                        handleToggleLeft(authority.id)
                                      }
                                      className="mt-0.5"
                                    />
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex items-center justify-between px-4 py-3 bg-muted/50 rounded-lg mb-3">
                ...
                <span className="text-sm text-muted-foreground">
                  {Object.keys(availableGrouped).length}개 카테고리 /{" "}
                  {availableAuthorityList.length}개 권한
                </span>
              </div>
              <div className="border rounded-lg flex-1 overflow-y-auto">
                {availableAuthorityList.length === 0 ? (
                  ...
                  <div className="p-3 space-y-3">
                    {Object.entries(availableGrouped).map(
                      ([categoryId, authorities]) => {
                        const isExpanded =
                          expandedRightCategories.has(categoryId);
                        const categoryName = getCategoryName(authorities);
                        return (
                          <div key={categoryId} className="space-y-1">
                            <button
                            ...
                            {isExpanded && (
                              <div className="space-y-1 ml-4">
                                {authorities.map((authority) => (
                                  ...
                                  >
                                    <Checkbox
                                      checked={selectedRightItems.has(
                                        authority.id,
                                      )}
                                      onCheckedChange={() =>
                                        handleToggleRight(authority.id)
                                      }
                                      className="mt-0.5"
                                    />
                                    <div className="flex-1 min-w-0">
...
// 조직 권한 할당 다이얼로그 컴포넌트
function OrgAuthorityAssignmentDialog({
  open,
  onOpenChange,
  orgId,
  orgName,
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [authorities, currentAuthorities] = await Promise.all([
        adminApi.getAllAuthorities(),
        adminApi.getOrganizationAuthorities(orgId),
      ]);
      setAllAuthorities(authorities);
      // 조직이 현재 가지고 있는 권한을 선택된 상태로 초기화
      setSelectedAuthorities(
        new Set(currentAuthorities.map((a) => a.authorityId)),
      );
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await adminApi.updateOrganizationAuthorities(
        orgId,
        Array.from(selectedAuthorities),
      );
      ...
      toast.success(`${orgName} 팀 권한이 업데이트되었습니다.`);
    } catch (error) {
      console.error("Failed to update org authorities:", error);
      toast.error("권한 업데이트에 실패했습니다.");
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[98vw] w-[98vw] max-h-[98vh] h-[98vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-xl">{orgName} 팀 권한 할당</DialogTitle>
          <DialogDescription>
            이 조직 소속 팀원들이 자동으로 상속받을 권한을 설정합니다.
          </DialogDescription>
        </DialogHeader>
...
function OrgSyncDialog({
  ...
  const [orgAuthorities, setOrgAuthorities] = useState<
    Array<{
      authorityId: number;
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    adminApi
      .getOrganizationAuthorities(orgId)
      .then((data) => setOrgAuthorities(data))
      .catch(() => toast.error("팀 권한 목록을 불러오지 못했습니다."))
  ...
  const handleSync = async () => {
    try {
      setIsSyncing(true);
      const result = await adminApi.syncOrganizationAuthorities(orgId);
      toast.success(result.message);
      onOpenChange(false);
    } catch {
      toast.error("권한 동기화에 실패했습니다.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  // 카테고리별 그룹핑
  const grouped = orgAuthorities.reduce<Record<string, typeof orgAuthorities>>(
    (acc, auth) => {
      const category = auth.authorityName.split(":")[0] || "기타";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            <span className="font-semibold text-foreground">{orgName}</span>에
            설정된 권한을 소속 팀원 전원에게 즉시 부여합니다. 기존 개인 권한은
            유지되며, 없는 권한만 추가됩니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 py-2">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">
                {orgAuthorities.length}개
              </span>
            </div>

            <div className="border rounded-lg h-72 overflow-y-auto bg-muted/30">
              {isLoading ? (
                <div className="flex items-center justify-center h-full text-muted-foreground text-sm">
                  로딩 중...
                </div>
              ) : orgAuthorities.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm gap-2 px-4 text-center">
                  <Shield className="w-8 h-8 opacity-30" />
                  <p>설정된 팀 권한이 없습니다.</p>
                  <p className="text-xs">팀 권한 할당에서 먼저 설정해주세요.</p>
                </div>
              ) : (
                <div className="p-3 space-y-3">
                  {Object.entries(grouped).map(([category, auths]) => (
                    <div key={category}>
                      ...
                      <div className="space-y-1">
                        {auths.map((auth) => (
                          <div
                            ...
                            className="flex items-start gap-2 px-2 py-1.5 rounded-md bg-background border border-border"
                          >
          ...
          <div className="flex flex-col gap-2">
            <p className="text-sm font-medium text-foreground flex items-center gap-1.5">
              <UsersIcon className="w-4 h-4 text-primary" />팀 정보
            </p>

            <div className="border rounded-lg h-72 overflow-y-auto bg-muted/30 p-3 space-y-3">
              {/* 팀 이름 + 인원 수 */}
              <div className="flex items-center justify-between px-3 py-2.5 rounded-md bg-background border border-border">
        ...
        {orgAuthorities.length > 0 && orgMembers.length > 0 && (
...
function OrganizationTreeNode({
  ...
  const [isOrgAuthorityDialogOpen, setIsOrgAuthorityDialogOpen] =
    useState(false);
  const [isOrgSyncDialogOpen, setIsOrgSyncDialogOpen] = useState(false);
  const [isAuthorityDialogOpen, setIsAuthorityDialogOpen] = useState(false);
  const [selectedUserForAuthority, setSelectedUserForAuthority] = useState<{
    id: number;
    username: string;
  } | null>(null);
  ...
  const availableUsers = users.filter(
    (user) => !user.organizationId || user.organizationId !== org.id,
  );
  ...
  const handleAddMembers = async () => {
    try {
      ...
      await adminApi.updateUsersOrganization(
      ...
      toast.success(
        `${selectedUserIds.size}명의 사용자를 ${org.name}에 추가했습니다.`,
      );
    ...
    <div className="ml-4">
      <ContextMenu>
        ...
        <ContextMenuContent className="w-48">
          ...
          <ContextMenuItem onClick={() => setIsOrgAuthorityDialogOpen(true)}>
            <Shield className="mr-2 h-4 w-4" />
            <span>팀 권한 할당</span>
          </ContextMenuItem>
          <ContextMenuItem onClick={() => setIsOrgSyncDialogOpen(true)}>
      ...
      {/* 조직 권한 할당 다이얼로그 */}
      <OrgAuthorityAssignmentDialog
        open={isOrgAuthorityDialogOpen}
        onOpenChange={setIsOrgAuthorityDialogOpen}
        orgId={org.id}
        orgName={org.name}
      />
      ...
      {/* 팀원 권한 동기화 다이얼로그 */}
      <OrgSyncDialog
        open={isOrgSyncDialogOpen}
        onOpenChange={setIsOrgSyncDialogOpen}
        orgId={org.id}
        orgName={org.name}
        org={org}
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {selectedUser.organizationId
                      ? organizations.find(
                          (o) => o.id === selectedUser.organizationId,
                        )?.name || "알 수 없음"
                      : "미소속"}
                  </p>
                </div>
