...
import {
  ...
  useSaveDbMutation,
  useDeleteDbMutation,
  useCreateDbFolderMutation,
  useRenameDbFolderMutation,
  useDeleteDbFolderMutation,
} from "@/features/db/useDb";
import { dbApi } from "@/entities/db/dbApi";
import type { DbFolder, DbPost, DbBlock } from "@/entities/db/db.types";
import { buildDbTree } from "@/entities/db/db.types";
import DbBlockEditor from "@/features/db/components/DbBlockEditor";
import DbBlockViewer from "@/features/db/components/DbBlockViewer";
...
export function DbPage() {
  ...
  const { data: folders = [] } = useDbFolders();
  ...
  const { data: postDetail } = useDbPostDetail(selectedPostId, !isEditing);

  const saveMutation = useSaveDbMutation(selectedPostId, (newId: number) => {
    setSelectedPostId(newId);
    setIsEditing(false);
  });

  const deleteMutation = useDeleteDbMutation(() => {
    setSelectedPostId(null);
    setIsEditing(false);
  });

  const createFolderMutation = useCreateDbFolderMutation(
    (parentId: number | null) => {
  ...
  const renameFolderMutation = useRenameDbFolderMutation(() => {
    setEditingFolderId(null);
  });

  const deleteFolderMutation = useDeleteDbFolderMutation(() => {
    setSelectedFolderId(null);
    setSelectedPostId(null);
  });

  const createDocMutation = useSaveDbMutation(null, (newId: number) => {
  ...
  // 폴더 렌더링
  const renderFolder = (folder: DbFolder, depth = 0) => {
    ...
    return (
      <div key={folder.id} className={depth > 0 ? "ml-4" : ""}>
        ...
        >
          ...
          {isEditingThis ? (
            <input
              ...
              className="flex-1 border rounded px-1 py-0 text-xs min-w-0 text-foreground bg-background border-border"
            />
          ) : (
        ...
        {isExpanded && (
          <>
            {subFolders.map((sub: DbFolder) => renderFolder(sub, depth + 1))}
    ...
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="bg-card rounded border mb-4">
        <div className="p-3 border-b bg-muted/30">
          <h1 className="text-lg font-bold text-foreground">DB 관리</h1>
        </div>
      </div>
                  ...
                  <div>
                    ...
                    {allPosts.length > 0 && (
                      <>
                        ...
                        {allPosts.map((p: DbPost & { folderId: number }) => (
                          <div
                            key={p.id}
        ...
        <div className="flex-1 min-w-0 bg-card rounded border">
          ...
          <div className="p-4">
            {isEditing ? (
              ...
              <DbBlockViewer post={postDetail} />
            ) : (
              <div className="flex items-center justify-center h-[300px] text-muted-foreground text-sm">
                {selectedFolderId
                  ? "폴더 hover 후 + 버튼으로 새 DB 문서 추가"
