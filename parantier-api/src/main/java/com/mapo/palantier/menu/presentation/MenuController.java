...
public class MenuController {
    ...
    @Operation(
        summary = "메뉴 트리 조회",
        description = "현재 사용자 권한에 맞는 메뉴 트리 조회"
    )
    @GetMapping("/tree")
    public ResponseEntity<List<MenuResponse>> getMenuTree(Authentication auth) {
        String role = "ROLE_USER"; // 기본값 (ROLE_ 접두사 포함)
    ...
    @Operation(
        summary = "자식 메뉴 조회",
        description = "특정 부모 메뉴의 자식 메뉴들 조회"
    )
    ...
    public ResponseEntity<MenuResponse> updateMenu(
        ...
        Menu updated = menuService.updateMenu(
            Menu.builder()
                .id(id)
                .name(menu.getName())
                .path(menu.getPath())
    ...
    @Operation(
        ...
        description = "메뉴 순서를 일괄 변경합니다"
    )
    @PatchMapping("/reorder")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> reorderMenus(
        @RequestBody List<MenuReorderRequest> requests
    ) {
        menuService.reorderMenus(requests);
