...
public class SubutaiDocController {
    ...
    @PostMapping("/folders")
    public ResponseEntity<Void> createFolder(@RequestBody SubutaiDocFolderRequest req,
    ...
    public ResponseEntity<Void> updateFolder(@PathVariable Long id,
    ...
    public ResponseEntity<List<SubutaiDocPost>> getPosts(@PathVariable Long folderId) {
    ...
    @PostMapping("/posts")
    public ResponseEntity<Long> createPost(@RequestBody SubutaiDocPostRequest req,
            @AuthenticationPrincipal User user) {
        return ResponseEntity.ok(docService.createPost(req, user != null ? user.getId() : 1L));
    }
