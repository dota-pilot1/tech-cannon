...
public class WorkspaceController {
    ...
    @Operation(
        summary = "노드 트리 조회",
        description = "사용자의 전체 노드 트리 조회"
    )
    @GetMapping("/tree")
    public ResponseEntity<List<WorkspaceNode>> getNodeTree(
        Authentication auth
    ) {
    ...
    @Operation(
        summary = "모든 노드 조회 (플랫)",
        description = "사용자의 모든 노드를 플랫 리스트로 조회"
    )
    @GetMapping("/nodes")
    public ResponseEntity<List<WorkspaceNode>> getAllNodes(
        Authentication auth
    ) {
        Long userId = getUserIdFromAuth(auth);
    ...
    @Operation(
        summary = "자식 노드 조회",
        description = "특정 노드의 자식 노드들 조회"
    )
    @GetMapping("/nodes/{id}/children")
    public ResponseEntity<List<WorkspaceNode>> getChildNodes(
    ...
    @Operation(
        summary = "노드 생성",
        description = "새로운 노드 생성 (폴더/태스크/문서)"
    )
    @PostMapping("/nodes")
    ...
    @Operation(
        summary = "노드 이동",
        description = "노드를 다른 부모로 이동 (드래그앤드롭)"
    )
    ...
    @Operation(
        summary = "노드 순서 재정렬",
        description = "같은 레벨의 노드들 순서 재정렬"
    )
    @PutMapping("/nodes/reorder")
    public ResponseEntity<Void> reorderNodes(
        @RequestBody ReorderNodesRequest request
    ) {
        workspaceService.reorderNodes(
            request.getParentId(),
            request.getNodeIds()
        );
        return ResponseEntity.noContent().build();
    ...
    @Operation(
        summary = "노드 삭제",
        description = "노드 삭제 (자식 노드도 함께 삭제)"
    )
