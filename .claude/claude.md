...
**⚠️ 중요: 사용자가 "커밋 푸시" 또는 유사한 요청을 하면 자동으로 commit과 push를 한 번에 수행합니다.**

### 자동화 정책
- 사용자 확인 없이 자동으로 `git add -A && git commit && git push` 실행
- 단계별로 물어보지 않고 한 번에 처리
- 커밋 메시지는 변경사항을 분석하여 자동 작성
